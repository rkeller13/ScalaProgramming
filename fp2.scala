...
object fp2 {
  ...
  def joinTerminateRight (xs : List[String], term : String) : String = {
    ...
    xs match {
      case Nil => ""
      case y::ys => y + foldRight(ys,term, (x:String,z:String)=>term+x+z)
    }
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    ...
    xs match {
      case Nil => ""
      case y::ys => y + foldLeft(ys, term, (x:String,z:String)=>x+z+term)
    }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def firstIndexHelper (a : Int, xs : List[Int], i : Int) : Int = {
      xs match {
        case Nil => throw new NoSuchElementException
        case y::ys => if (y >= a) i else firstIndexHelper(a,ys,(i+1))
     }
    }
    firstIndexHelper(a,xs,0)
  }
}
