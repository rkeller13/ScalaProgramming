...
//Robert Keller CSC 447 HW5 Storage 
...
object storage {
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
      ...
      /*() => {
        var counter = 0
        def count : Int => (() => Int, Int => Unit) = {
          n : Int => {
            var x = n
            def reader : () => Int = () => x
            def writer : Int => Unit = y => (counter = counter +1, x = y)
            (reader, writer)
          }
      ...
      //I could not figure this one out, so I commented out my code and included null so that it would compile
      null
  }
